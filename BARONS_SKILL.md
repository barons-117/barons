...
|---|---|---|
| `estimated_value` | ✅ **שווי נטו של החברה** | **NULL** |
| `asset_purchases` | ✅ רכישות החלקים ב-LLC | ❌ ריק |
| `asset_income` | ❌ ריק | ✅ שכירות ושוכרים |
| כתובת, תמונות, מסמכים | ❌ (רק כתובת רשומה) | ✅ |
| `asset_partners` | ✅ אחוז אפקטיבי | ✅ אותו אחוז אפקטיבי |

**כל השווי יושב על המטרייה, לבנים אין שווי כלל.** זה מה שמונע ספירה כפולה, ועובד בלי שום שינוי קוד.

**שתי דרכים תקפות לתמחר את המטרייה:**

1. **Cost basis** — `estimated_value = NULL`. ה-fallback ב-`SummaryBlock` וב-`AssetCard` מחשב אז `_totalPurchasesILS / internalPct × pct`, מה שמחזיר בדיוק את סך מה ששולם. פשוט, לא דורש עדכון, אבל לא משקף שווי אמיתי.
2. **שווי נטו** — `estimated_value` מוצב מפורשות. ברגע שהוא מוצב, ה-fallback **לא** מופעל (`hasEstimate = !!asset.estimated_value`) והרכישות נשארות כהיסטוריה בלבד. דורש עדכון ידני אחרי כל שמאות או שינוי משכנתא.

**בפרויקט זה נבחר שווי נטו** (אוגוסט 2026). הנוסחה:

```
שווי נכסים לפי שמאות  +  מזומן בחשבון  −  יתרת משכנתא  +  הזרמות הון
```

⚠️ `estimated_value = 0` **לא** עובד כדרך לנטרל שווי — אפס הוא falsy, ולכן `hasEstimate` יוצא false וה-fallback של הרכישות נכנס לפעולה בכל זאת. להשתמש ב-`NULL` בלבד.

**מלכודת:** אם מוסיפים `estimated_value` לנכס בן *וגם* משאירים שווי או רכישות על המטרייה — ספירה כפולה מיידית. השווי יושב במקום אחד בלבד.

**לא צריך** דגל `is_holding` ולא צריך לסנן את המטרייה מהחישוב. נשקל ונדחה — היה מוסיף סיבוכיות בלי תועלת.
...
a1000000-...-0004  OKY Properties LLC     equity · EIN 36-4901498 · 14.5598795%
                                          estimated_value = $645,000
├─ a1000000-...-0041  טווינסבורג — 11629-11635 Ravenna Rd   (הכנסה $5,700/חודש)
└─ a1000000-...-0042  קליבלנד — 2023 W 93rd St              (2 יח׳, is_active=false)
```

**החברה:** Ohio LLC, התאגדות 07/06/2018. NAICS 531110. שותף מספר 6.
Registered Agent: Brett Youngs, 321 Center Rd, Bedford OH 44146.
דואר: 3401 Enterprise Pkwy Ste 340-483, Beachwood OH 44122.
מורשה חתימה יחיד: יובל קליין (ת"ז 200361996). Operating Agreement אחרון: 12/09/2024.

**בעלות (Exhibit A):** יובל 36.73% ($140,320) · מיכל 48.71% ($186,096) · ארז 14.56% ($55,619).

**רכישות על המטרייה:** $38,990 (2020) + $21,745 (2021) + $9,463 (2026) = **$70,198**.

**שווי נטו (22/02/2026):** Ravenna 550,000 + 93rd 300,000 + מזומן 3,034 − משכנתא 273,011 + הזרמה 65,000 = **$645,000**. החלק שלנו: $93,912.

**משכנתא:** Lendai US Inc, $275,000, ריבית 8.7215%, 360 חודשים, החזר $2,157.83/חודש. שעבוד ראשון על 11635 Ravenna בלבד. ערבים אישיים: יובל ומיכל. עמלת פתיחה $11,000. קנס פירעון מוקדם 5% יורד לאפס אחרי 5 שנים. כספי המשכנתא מימנו את רכישת 93rd.

**אנשי קשר:** Avalon Group Realty (ניהול Ravenna, 10% דמי ניהול) · מיכה פז / M. Moretzky LLC (רו"ח אמריקאי) · אורן סמילה (Lendai) · MAOR IC Group + Billi Kas (שיפוץ 93rd) · Elite Commercial Closings (סגירה).

**מקורות נתונים:** K-1 שנתי מ-M. Moretzky (מגיע ~יולי), טופס 8805, Schedule K-3, Ohio IT K-1. מס עירוני ל-RITA עבור Twinsburg בלבד. מס רכוש: Summit County.

**שיטת ההשקעה — BRRRR:** Buy → Renovate → Rent → Refinance → Repeat. מימון של כ-60% מהשווי המשוחזר מממן את הנכס הבא.

**פתוח:**
- לוודא עם יובל שהאחוז נשאר 14.56% אחרי הזרמת ה-$65,000. בטבלה מ-22/02/2026 הוא חישב 19.29%, אבל מיכל דחתה את העיגול והועבר לפי האחוזים הקיימים. ה-K-1 של 2025 מאשר 14.5598795% אך לא מכסה את 2026
- יחידה 2 ב-93rd לא מפורסמת ב-Zillow — $1,900/חודש שיושב על השולחן
- תמונות 93rd עדיין לא הועלו (יש 27 תמונות ו-2 סרטונים בארכיון הוואטסאפ מ-21/03/2025)
- כששוכר נחתם — `is_active = true` על רשומת ההכנסה הרלוונטית
...
4. לעדכן את `estimated_value` **על המטרייה** בשווי הנכס החדש
5. אימות: `sum_pct = 1.0000`, `estimated_value IS NULL` על הבן, אפס רכישות על הבן
6. תמונות — ידנית דרך ה-UI (לא ניתן דרך SQL)

### מבנה התיאורים

`description` מרונדר עם `whiteSpace: 'pre-wrap'` ב-`GeneralSection`, כך ששורות ריקות
ומבנה נשמרים. התבנית שנבחרה: ראשי פרקים מופרדים ב-`── כותרת ──`, וטבלאות מספרים
מיושרות ברווחים עם קו `─────────` לפני שורת הסיכום.

חלוקת הכותרות בפועל:
- **חברה:** פרטי החברה · מבנה בעלות · איך הגענו לכאן · ניהול ודיווח · שיטת ההשקעה · מסמכים · שווי נטו
- **נכס:** הנכס · שמאות · משכנתא · שכירות · ניהול · בעלות
- **נכס בשיפוץ:** הנכס · עלות הרכישה · הערכת שווי · תחזית הכנסה · השיפוץ · בעלות

אורך סביר: 1,600–2,700 תווים לנכס. מעבר לזה הבלוק נעשה כבד בעמוד.
