...
function ImpressionsModal({ trip, onClose, onSaved }) {
  ...
  return (
    <Modal title="רשמים ופרטי טיול" onClose={onClose}>
      ...
      <Field label="רשמים"><textarea style={{...inp,minHeight:'220px',resize:'vertical',lineHeight:'1.7'}} value={text} onChange={e=>setText(e.target.value)} placeholder="כתוב את הרשמים שלך..." onFocus={focusInp} onBlur={blurInp}/></Field>
      <SaveBtn loading={loading} onClick={save}/>
    </Modal>
  )
}
                ...
                {sortedFlightsArr.length === 0
                  ...
                  : sortedFlightsArr.map((f, fIdx) => (
                    <div key={f.id} className="td-flight-row" style={{
                      ...
                      {/* Xtra */}
                      <div className="td-flight-extras" style={{ display:'flex',flexDirection:'column',alignItems:'flex-end',gap:'4px',fontSize:'11px',color:LT.muted }}>
