    ...
    <div style={S.page}>
      ...
      <header style={S.header}>
        <nav style={{display:'flex',alignItems:'center',gap:'6px',fontSize:'13px',color:'#64748b'}}>
          <button style={S.crumbBtn} onClick={()=>navigate('/')}>BARONS</button>
          <span style={{color:'#cbd5e1'}}>/</span>
          <span style={{color:'#1e293b',fontWeight:600}}>נסיעות</span>
        </nav>
        <div style={{display:'flex',gap:'10px',alignItems:'center'}}>
          <button style={S.statsBtn} onClick={()=>navigate('/stats')}>סטטיסטיקות</button>
          <button style={S.addBtn} onClick={()=>setShowAdd(true)}>+ נסיעה חדשה</button>
          <button style={S.logoutBtn} onClick={async()=>{await supabase.auth.signOut();navigate('/')}}>יציאה</button>
        </div>
      </header>
