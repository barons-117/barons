  ...
  return (
    <div style={{
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom: 16 }}>
        <span style={{ fontSize:13, fontWeight:700, color:'rgba(255,255,255,0.5)', letterSpacing:'1px', textTransform:'uppercase' }}>
          {title}
        </span>
        {action}
      </div>
...
function EditBtn({ onClick, hidden }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      color:'rgba(255,255,255,0.45)', cursor:'pointer', fontFamily:'Heebo,sans-serif',
...
function SaveBtn({ onClick, loading }) {
  return (
    <button onClick={onClick} disabled={loading} style={{
      ...
      background:'#1d4ed8', color:'white', cursor:'pointer', fontFamily:'Heebo,sans-serif',
    }}>{loading ? '...' : 'שמור'}</button>
  )
}
...
function CancelBtn({ onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      border:'1px solid rgba(255,255,255,0.15)', background:'transparent',
      color:'rgba(255,255,255,0.4)', cursor:'pointer', fontFamily:'Heebo,sans-serif',
    }}>ביטול</button>
  )
}
  ...
  return (
    <input type={type} value={value || ''} onChange={e => onChange(e.target.value)}
      ...
      style={{
        ...
        fontFamily:'Heebo,sans-serif', outline:'none', width:'100%', boxSizing:'border-box',
        ...style,
      }}
    />
  )
  ...
  return (
    <select value={value || ''} onChange={e => onChange(e.target.value)}
      style={{
        ...
        borderRadius:8, padding:'8px 12px', fontSize:13, color:'white',
        fontFamily:'Heebo,sans-serif', outline:'none', ...style,
      }}
    >
      {options.map(o => (
...
function Row({ label, value, muted }) {
  return (
    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start',
      ...
      <span style={{ fontSize:12, color:'rgba(255,255,255,0.35)', minWidth:100 }}>{label}</span>
      <span style={{ fontSize:13, color: muted ? 'rgba(255,255,255,0.35)' : 'white', textAlign:'left', flex:1, marginRight:12 }}>{value || '—'}</span>
    </div>
  )
}
...
function CoverImage({ asset, onSave }) {
    ...
    <div style={{ marginTop:14, paddingTop:14, borderTop:'1px solid rgba(255,255,255,0.06)' }}>
      {urls.length === 0 ? (
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <button onClick={() => setEditing(true)} style={Cs.editSmall}>+ תמונת נכס</button>
        </div>
      ) : (
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          ...
          <button onClick={() => setEditing(true)} style={Cs.editSmall}>עריכה</button>
        </div>
      )}
  ...
  return (
    <div style={{ marginTop:14, paddingTop:14, borderTop:'1px solid rgba(255,255,255,0.06)' }}>
      ...
      <button onClick={() => setEditing(false)} style={Cs.editSmall}>סיום עריכה</button>
    </div>
  )
}
...
function GeneralSection({ asset: assetProp, onSave, readOnly }) {
  ...
  if (editing) return (
    <SectionCard title="מידע כללי" action={
  ...
  return (
    <SectionCard title="מידע כללי" action={<EditBtn onClick={startEdit} hidden={readOnly} />}>
...
function PartnersSection({ assetId, partners, onSave, readOnly }) {
  const [editing, setEditing] = useState(false)
  ...
  if (editing) return (
    <SectionCard title="שותפים" action={
      <div style={{ display:'flex', gap:8, alignItems:'center' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'160px 1fr 72px 1fr 24px', gap:8, marginBottom:6 }}>
        <span style={{ fontSize:10, color:'rgba(255,255,255,0.3)' }}>ישות</span>
        <span style={{ fontSize:10, color:'rgba(255,255,255,0.3)' }}>שם (לחיצוני)</span>
        <span style={{ fontSize:10, color:'rgba(255,255,255,0.3)' }}>אחוז</span>
        <span style={{ fontSize:10, color:'rgba(255,255,255,0.3)' }}>הערה</span>
        <span />
      </div>
      {rows.map((row, i) => {
        ...
        return (
          <div key={i} style={{ display:'grid', gridTemplateColumns:'160px 1fr 72px 1fr 24px', gap:8, marginBottom:8, alignItems:'center' }}>
            ...
            <div style={{ position:'relative' }}>
              <select
                ...
                style={{
                  ...
                  fontFamily:'Heebo,sans-serif', outline:'none', fontWeight:700, cursor:'pointer',
                }}
              >
                {ENTITIES.map(e => (
            ...
            <button onClick={() => setRows(r => r.filter((_,j) => j!==i))}
              style={{ background:'none', border:'none', color:'rgba(255,255,255,0.2)', cursor:'pointer', fontSize:18, padding:0, lineHeight:1 }}
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:8 }}>
        <button onClick={addRow} style={{ fontSize:12, color:'#60a5fa', background:'none', border:'none', cursor:'pointer', fontFamily:'Heebo,sans-serif', padding:0 }}>
          + הוסף שותף
        </button>
        {!totalOk && editTotal > 0 && (
  ...
  return (
    <SectionCard title="שותפים" action={<EditBtn onClick={startEdit} hidden={readOnly} />}>
      <div style={{ display:'flex', gap:20, alignItems:'center', flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1, minWidth:180 }}>
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.08)', marginTop:8, paddingTop:8,
            fontSize:12, color:'rgba(255,255,255,0.3)', display:'flex', justifyContent:'space-between' }}>
            <span>סה"כ</span>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:4 }}>
        <div>
          ...
          {(inc.tenant_phone2 || inc.tenant_email2) && (
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:1 }}>
              {inc.tenant_name2 && <span style={{ color:'rgba(255,255,255,0.2)', marginLeft:4 }}>{inc.tenant_name2}:</span>}
        ...
        <div style={{ textAlign:'left', minWidth:180 }}>
          ...
          <div style={{ fontSize:15, fontWeight:700, color:'white', marginBottom:2 }}>
            ...
            <span style={{ fontSize:11, color:'rgba(255,255,255,0.4)', fontWeight:400, marginRight:4 }}>
              {VAT_LABELS[inc.vat_type]} · {FREQ_LABELS[inc.payment_frequency]}
            </span>
          </div>
          {inc.currency !== 'ILS' && (
            <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', textAlign:'left', marginBottom:2 }}>
              ≈ {fmtILS(monthly)}/חודש
            </div>
          )}
    ...
    <div style={{ marginTop:8 }}>
      <button
        ...
        style={{
          ...
          fontSize:12, color:'rgba(255,255,255,0.3)', fontFamily:'Heebo,sans-serif',
            ...
            return (
              <div key={inc.id || i} style={{ marginBottom:6 }}>
                <button
                  ...
                  style={{
                    background:'none', border:'none', cursor:'pointer',
                    fontFamily:'Heebo,sans-serif', padding:0, textAlign:'right', width:'100%',
                    display:'flex', alignItems:'center', justifyContent:'space-between',
                  }}
                >
                  <span style={{ fontSize:13, color:'rgba(255,255,255,0.45)', fontWeight:600 }}>{names}</span>
                ...
                {isExp && (
                  <div style={{ fontSize:12, color:'rgba(255,255,255,0.35)', marginTop:6, lineHeight:1.8 }}>
...
function IncomeSection({ assetId, income, partners, fx, onSave, readOnly }) {
  const [editing, setEditing] = useState(false)
  const [rows, setRows]       = useState([])
  const [saving, setSaving]   = useState(false)
  ...
  if (editing) return (
    <SectionCard title="הכנסות ושוכרים" action={
      <div style={{ display:'flex', gap:8 }}>
        <CancelBtn onClick={() => setEditing(false)} />
        <SaveBtn onClick={save} loading={saving} />
      </div>
      ...
      {rows.map((row, i) => (
        <div key={i} style={{ background:'rgba(255,255,255,0.03)', borderRadius:10, padding:'14px', marginBottom:12, border:'1px solid rgba(255,255,255,0.07)' }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <button onClick={() => setRows(r => r.filter((_,j) => j!==i))}
              style={{ background:'none', border:'none', color:'#f87171', cursor:'pointer', fontSize:16, marginTop:16 }}>×</button>
          </div>
        </div>
      ))}
      <button onClick={addRow} style={{ fontSize:12, color:'#60a5fa', background:'none', border:'none', cursor:'pointer', fontFamily:'Heebo,sans-serif', padding:0 }}>
  ...
  return (
    <SectionCard title="הכנסות ושוכרים" action={<EditBtn onClick={startEdit} hidden={readOnly} />}>
      {income.length === 0 && <div style={{ color:'rgba(255,255,255,0.25)', fontSize:13 }}>אין הכנסות רשומות</div>}
      {/* שוכרים פעילים */}
      ...
      {income.length > 0 && (
        <div style={{ marginTop:12, paddingTop:10, borderTop:'1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
            <span style={{ fontSize:12, color:'rgba(255,255,255,0.35)' }}>סה"כ מהנכס</span>
...
function PurchasesSection({ assetId, purchases, onSave, readOnly }) {
  const [editing, setEditing] = useState(false)
  const [rows, setRows]       = useState([])
  const [saving, setSaving]   = useState(false)
  ...
  if (editing) return (
    <SectionCard title="היסטוריית רכישה" action={
      <div style={{ display:'flex', gap:8 }}>
        <CancelBtn onClick={() => setEditing(false)} />
        <SaveBtn onClick={save} loading={saving} />
      </div>
      ...
      <button onClick={() => setRows(r => [...r, { id:null, purchase_date:'', amount:'', currency:'ILS', from_whom:'', notes:'' }])}
        style={{ fontSize:12, color:'#60a5fa', background:'none', border:'none', cursor:'pointer', fontFamily:'Heebo,sans-serif', padding:0, marginTop:4 }}>
        + הוסף רכישה
      </button>
    </SectionCard>
  )
  ...
  return (
    <SectionCard title="היסטוריית רכישה" action={<EditBtn onClick={startEdit} hidden={readOnly} />}>
      {purchases.length === 0 && <div style={{ color:'rgba(255,255,255,0.25)', fontSize:13 }}>אין נתוני רכישה</div>}
      {purchases.map((p, i) => (
        <div key={p.id || i} style={{ padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
          <div style={{ display:'flex', justifyContent:'space-between' }}>
            <div>
              ...
              {p.from_whom && <span style={{ fontSize:12, color:'rgba(255,255,255,0.4)', marginRight:8 }}>מ: {p.from_whom}</span>}
            </div>
            <span style={{ fontSize:12, color:'rgba(255,255,255,0.35)' }}>{fmtDate(p.purchase_date)}</span>
          </div>
          {p.notes && <div style={{ fontSize:11, color:'rgba(255,255,255,0.3)', marginTop:3 }}>{p.notes}</div>}
        </div>
      ))}
    </SectionCard>
  )
...
function EventsSection({ assetId, events, onSave, readOnly }) {
  ...
  return (
    <SectionCard title="לוג אירועים" action={
      ...
      {sorted.length === 0 && <div style={{ color:'rgba(255,255,255,0.25)', fontSize:13 }}>אין אירועים רשומים</div>}
      {sorted.map((ev, i) => (
        <div key={ev.id || i} style={{ display:'flex', gap:14, padding:'10px 0', borderBottom:'1px solid rgba(255,255,255,0.05)', alignItems:'flex-start' }}>
          <div style={{ minWidth:80, fontSize:11, color:'rgba(255,255,255,0.3)', paddingTop:2 }}>{fmtDate(ev.event_date)}</div>
          <div style={{ flex:1, fontSize:13, color:'rgba(255,255,255,0.8)', lineHeight:1.6 }}>{ev.description}</div>
          <button onClick={() => deleteEvent(ev.id)}
            style={{ background:'none', border:'none', color:'rgba(255,255,255,0.15)', cursor:'pointer', fontSize:14, padding:0, flexShrink:0 }}
            onMouseEnter={e => e.currentTarget.style.color='#f87171'}
...
function ContactsSection({ assetId, contacts, onSave, readOnly }) {
  const [editing, setEditing] = useState(false)
  const [rows, setRows]       = useState([])
  const [saving, setSaving]   = useState(false)
  ...
  if (editing) return (
    <SectionCard title="אנשי קשר" action={
      <div style={{ display:'flex', gap:8 }}>
        <CancelBtn onClick={() => setEditing(false)} />
        <SaveBtn onClick={save} loading={saving} />
      </div>
  ...
  return (
    <SectionCard title="אנשי קשר" action={<EditBtn onClick={startEdit} hidden={readOnly} />}>
      {contacts.length === 0 && <div style={{ color:'rgba(255,255,255,0.25)', fontSize:13 }}>אין אנשי קשר</div>}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(220px,1fr))', gap:10 }}>
        {contacts.map((c, i) => (
          <div key={c.id || i} style={{ background:'rgba(255,255,255,0.04)', borderRadius:10, padding:'12px 14px' }}>
            <div style={{ fontSize:13, fontWeight:700, color:'white', marginBottom:2 }}>{c.name}</div>
            {c.role  && <div style={{ fontSize:11, color:'rgba(255,255,255,0.4)', marginBottom:6 }}>{c.role}</div>}
...
function FilesSection({ assetId, files, onSave, readOnly }) {
  const [editing,     setEditing]     = useState(false)
  const [uploading,   setUploading]   = useState(false)
  const [lightbox,    setLightbox]    = useState(null)
  ...
  if (!editing) return (
    <>
      {lightbox && <Lightbox url={lightbox} onClose={() => setLightbox(null)} />}
      <SectionCard title="קבצים ומסמכים" action={
        <EditBtn onClick={() => setEditing(true)} hidden={readOnly} />
      }>
        {files.length === 0 && (
          <div style={{ color:'rgba(255,255,255,0.2)', fontSize:13, textAlign:'center', padding:'12px 0' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(110px,1fr))', gap:8, marginBottom: docs.length ? 14 : 0 }}>
            {images.map(f => (
              <div key={f.id} onClick={() => openFile(f)} style={{ cursor:'pointer', borderRadius:8, overflow:'hidden', border:'1px solid rgba(255,255,255,0.08)' }}>
                <ImageThumb file={f} />
                <div style={{ padding:'4px 8px', fontSize:11, color:'rgba(255,255,255,0.35)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                  {f.caption || '—'}
                </div>
              </div>
            ))}
        ...
        {docs.map(f => (
          <div key={f.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.05)', cursor:'pointer' }}
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              <div style={{ fontSize:10, color:'rgba(255,255,255,0.25)' }}>{FILE_TYPE_LABELS[f.file_type]}</div>
            </div>
      ...
      <SectionCard title="קבצים ומסמכים" action={
        <div style={{ display:'flex', gap:8 }}>
          ...
          <label style={{
            ...
            cursor: uploading ? 'default' : 'pointer', fontFamily:'Heebo,sans-serif',
          }}>
            {uploading ? 'מעלה...' : '+ קובץ'}
          ...
          <button onClick={() => { setEditing(false); setShowUrlInput(false) }}
            style={{ fontSize:11, padding:'4px 12px', borderRadius:8,
              ...
              color:'white', cursor:'pointer', fontFamily:'Heebo,sans-serif' }}>
            סיום
          </button>
        </div>
      }>
        {/* URL panel */}
        {showUrlInput && (
          <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:10, padding:14, marginBottom:12, border:'1px solid rgba(255,255,255,0.08)' }}>
        ...
        <div
          ...
          style={{
            ...
            transition:'all 0.15s', marginBottom: files.length ? 14 : 0,
          }}
        >גרור קבצים לכאן</div>
                  ...
                  {editName?.id === f.id ? (
                    <input autoFocus value={editName.value}
                      ...
                      style={{ width:'100%', background:'transparent', border:'none',
                        ...
                        fontSize:11, fontFamily:'Heebo,sans-serif', outline:'none' }} />
                  ) : (
                    <div onClick={() => setEditName({ id:f.id, value:f.caption||'' })}
                      style={{ fontSize:11, color:'rgba(255,255,255,0.4)', cursor:'text',
                        overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                      {f.caption || 'כיתוב...'}
                    </div>
                  )}
                </div>
                <button onClick={() => deleteFile(f)} style={{
        ...
        {docs.map(f => (
          <div key={f.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 0', borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              {editName?.id === f.id ? (
                <input autoFocus value={editName.value}
                  ...
                  style={{ width:'100%', background:'transparent', border:'none',
                    borderBottom:'1px solid rgba(255,255,255,0.3)', color:'white',
                    fontSize:13, fontFamily:'Heebo,sans-serif', outline:'none' }} />
              ) : (
                <div onClick={() => setEditName({ id:f.id, value:f.caption||'' })}
            ...
            <button onClick={() => deleteFile(f)}
              style={{ background:'none', border:'none', color:'rgba(255,255,255,0.2)', cursor:'pointer', fontSize:18, padding:0, flexShrink:0 }}
              onMouseEnter={e => e.currentTarget.style.color='#f87171'}
              onMouseLeave={e => e.currentTarget.style.color='rgba(255,255,255,0.2)'}
            >×</button>
          </div>
...
function ImageThumb({ file, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      style={{
        height:100, background:'rgba(255,255,255,0.05)', cursor:'pointer',
        display:'flex', alignItems:'center', justifyContent:'center', overflow:'hidden',
      }}
    >
      {url
        ? <img src={url} style={{ width:'100%', height:'100%', objectFit:'cover' }} alt={file.caption} />
        : <span style={{ color:'rgba(255,255,255,0.2)', fontSize:11 }}>טוען...</span>
      }
    </div>
  )
}
    ...
    <div style={S.page}>
      ...
      <div style={S.shell}>
        <BaronsHeader
          ...
          actions={!isRoi ? [{ label: '✎ עריכה', onClick: () => setShowEdit && setShowEdit(true) }] : []}
        />
    </div>
  )
}
...
const L = {
  label: { fontSize:11, color:'rgba(255,255,255,0.35)', display:'block', marginBottom:4 }
}
...
const S = {
  ...
  shell: { width:'100%', maxWidth:800, padding:'36px 20px 80px', boxSizing:'border-box' },
}
